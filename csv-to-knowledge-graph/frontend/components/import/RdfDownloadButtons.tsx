...
export const RdfGenerationButtons: React.FC<RdfGenerationButtonsProps> = ({
  ...
  const baseFilename = currentItem.name
    .replace(/\s+/g, "-")
    .replace(".csv", "");
  ...
  const disableDataGeneration =
    isGeneratingData || !hasGraphData || !hasRdfTemplate;

  return (
    <div className="mt-6 bg-[#1c1c1c] rounded-lg border border-[#2a2a2a] p-6 mb-8">
      <h2 className="text-lg font-medium text-white mb-4">
        Generate & Download RDF Files
      </h2>
      <p className="text-sm text-gray-400 mb-4">
        Generate and download RDF template and data files without importing to
        Dgraph. You can use these files with other tools or save them for later
        use.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#222] p-5 rounded-lg border border-[#333]">
          ...
          <p className="text-sm text-gray-400 mb-4">
            The template defines how your CSV data is converted to a knowledge
            graph structure.
            {hasRdfTemplate && (
