...
import { ChevronRight, Upload, CheckCircle, AlertCircle, Copy, Database, Loader2, ArrowRight } from 'lucide-react';
...
export default function DgraphImportStep({ currentItem, onNextToQueries, isBatch = false }: DgraphImportStepProps) {
    ...
    <div className="max-w-5xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">Import the generated RDF data into your Dgraph instance</p>
          </div>
      ...
      {isImporting && (
        <div className="bg-[#1c1c1c] shadow rounded-lg border border-[#2a2a2a] p-6 mb-8">
          <h2 className="text-lg font-medium text-white mb-4">Import Progress</h2>
          <div className="mb-2">
            <div className="flex justify-between text-sm text-gray-300 mb-1">
              <span>{importStatus || `Importing ${isBatch ? 'batch ' : ''}RDF data to Dgraph...`}</span>
      ...
      <div className="mt-8 p-6 bg-blue-900/20 border border-blue-800/40 rounded-lg">
        <h3 className="text-sm font-medium text-blue-300 mb-2">Using this page</h3>
        <ul className="text-sm text-blue-200 space-y-1 list-disc pl-5">
          <li>Make sure your Dgraph instance is running and connected via the sidebar</li>
          <li>The "Import to Dgraph" button will send your RDF data to Dgraph</li>
          <li>If you just want to see example queries without importing, use "Skip to Queries"</li>
          <li>After import completes, you'll be able to explore your data with queries</li>
        </ul>
      </div>
    </div>
  );
