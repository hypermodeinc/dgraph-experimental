...
` && searchIndex++;
  ...
  */
  ...
  // src/index.js
  var serverTypeSelect = document.getElementById("serverType");
  ...
  var toolParams = document.getElementById("toolParams");
  var paramsContainer = document.getElementById("paramsContainer");
  var callToolBtn = document.getElementById("callToolBtn");
  var logElement = document.getElementById("log");
  var resultsElement = document.getElementById("results");
  var parsedJsonElement = document.getElementById("parsedJson");
  var mcpClient = null;
  var transport = null;
  var toolsInfo = [];
  var SERVER_PRESETS = {
  ...
  function debugLog(message, data) {
    const logEntry = {
      ...
      message,
      data: data !== void 0 ? typeof data === "string" ? data : JSON.stringify(data, null, 2) : null
    };
  ...
  function showResults(data, includeDebug = false) {
    ...
    if (includeDebug && debugLogs.length > 0) {
      content += "\n\n---- DEBUG LOGS ----\n";
      debugLogs.forEach((log2) => {
        content += `
[${log2.timestamp}] ${log2.message}`;
        if (log2.data) {
          content += `
${log2.data}`;
        }
  ...
  function updateDebugDisplay() {
    ...
    debugLogs.forEach((log2) => {
      ...
      header.textContent = `[${log2.timestamp}] ${log2.message}`;
      logEntry.appendChild(header);
      if (log2.data) {
        const data = document.createElement("pre");
        data.className = "debug-data";
        data.textContent = log2.data;
  ...
  async function connect() {
    try {
      ...
      toolsInfo.forEach((tool) => {
        ...
        if (tool.schema)
          debugLog("schema property:", tool.schema);
        if (tool.input_schema)
          debugLog("input_schema property:", tool.input_schema);
        if (tool.inputSchema)
          debugLog("inputSchema property:", tool.inputSchema);
      });
